...
export default class Login extends Component{
    ...
    async  componentDidMount(){
      await   firebase.database().ref('usuarios').on('value', (item)=>{
    ...
    render(){
        return(
            ...
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center'}}>
                <Image source={logo}  style={{width: 1000, height: 150, resizeMode: 'contain'}}/>
                <Image source={boneco}  style={{width: 1000, height: 150, resizeMode: 'contain'}}/>

                <TextInput
                value={this.state.user}
                onChangeText={ (text) => {this.setState({user: text})}}
                placeholder="Usuario"
                style={{width: '75%', borderRadius: 5, marginTop: 25, paddingLeft: 12, fontSize: 16, backgroundColor: 'white'}}
                >
                </TextInput>
                <TextInput
                ...
                <Botao
                    onPress={()=>{
                        ...
                        for(let i = 0; i<=(dados.usuarios.length-1); i++){
                            if(this.state.user == dados.usuarios[i].usuario  && this.state.senha == dados.usuarios[i].senha ){
                                dados.error = false,
                                this.setState({dados})
                                this.handlerStorage(dados.usuarios[i]);
                ...
                >
                    <Text style={{fontSize: 16, fontWeight: 'bold', fontStyle: 'normal', color: 'black'}}>ENTRAR</Text>
                </Botao>
                <TouchableOpacity style={{marginTop: 20, opacity: 0.5, color: '#FFF'}}
                ...
                <Portal>
                    ...
                    <Dialog.Content>
                    <Paragraph>{"Solicite sua senha no número\n(24) 99856-5959"}</Paragraph>
                    </Dialog.Content>
                    <Dialog.Actions>
                        <TouchableOpacity
                        color="black"
